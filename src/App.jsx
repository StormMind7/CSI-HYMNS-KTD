  ...
  const lines = (data.lines || []).map(line => {
    ...
    line.forEach((group, gi) => {
      if (Array.isArray(group)) {
        group.forEach((subGroup, sgi) => {
          const parts = String(subGroup).trim().split(/\s+/).filter(Boolean);
          parts.forEach((s, si) => {
            syls.push({ syl: s, wordBreak: (gi > 0 || sgi > 0) && si === 0 });
          });
        });
      } else {
...
function buildPrintHTML(hymns_, indices, lyricSize, chordSize, lineGap) {
  ...
  const buildHymn = (h) => {
    ...
    safe.lines.forEach((line, li) => {
      if (!line || !line.length) { html += `<div style="height:${lineGap*2}px;"></div>`; return; }
      ...
      line.forEach((sylObj, si) => {
        ...
        if (isBreak) {
          html += `<div class="word-gap" style="width:${wordGap}px;"><div style="height:${chordH}px;"></div><div>&nbsp;</div></div>`;
        }
        html += `<div class="syl-col">`;
        html += `<div class="chord" style="min-height:${chordH}px;">${chord}</div>`;
        html += `<div class="lyric">${syl}</div>`;
        html += `</div>`;
      });
      html += `</div>`;
    });
  ...
  indices.forEach((idx, i) => {
    const pb = i > 0 ? ' style="page-break-before:always;"' : '';
    body += `<div class="page"${pb}>${buildHymn(hymns_[idx])}</div>`;
  });
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .page { display: flex; flex-direction: column; align-items: center; padding: 8mm 0; width: 100%; }
  .hymn-block { width: 100%; display: flex; flex-direction: column; align-items: center; }
  .hymn-title { text-align: center; margin-bottom: ${lineGap * 2}px; font-family: 'Cinzel', Georgia, serif; font-size: ${titleSize}pt; font-weight: 700; color: #5a3000; }
  .hymn-key { font-family: 'Courier New', monospace; font-size: ${Math.round(titleSize * .62)}pt; color: #8a5500; }
  .line-row { display: flex; flex-direction: row; flex-wrap: wrap; align-items: flex-end; justify-content: center; margin-bottom: ${lineGap}px; }
  .word-gap { display: inline-flex; flex-direction: column; align-items: center; }
  .syl-col { display: inline-flex; flex-direction: column; align-items: center; padding: 0 ${Math.round(lyricSize * .15)}px; }
  .chord { font-family: 'Courier New', monospace; font-size: ${chordSize}pt; font-weight: 700; color: #8B4500; display: flex; align-items: flex-end; justify-content: center; white-space: nowrap; padding-bottom: 1px; min-width: 12px; }
  .lyric { font-family: 'Noto Sans Kannada', 'Arial Unicode MS', Georgia, sans-serif; font-size: ${lyricSize}pt; color: #111; line-height: 1.4; white-space: nowrap; }
...
function doPDFExport(hymns_, indices, lyricSize, chordSize, lineGap) {
  const html = buildPrintHTML(hymns_, indices, lyricSize, chordSize, lineGap);
  const win = window.open("", "_blank", "width=900,height=700");
  if (!win) { alert("Please allow pop-ups for this site to print/save PDF."); return; }
...
//  DOCX EXPORT
// ─────────────────────────────────────────────────────────────
async function doDOCXExport(hymns_, indices) {
  const children = [];
  indices.forEach((idx, pageIdx) => {
    const h = ensureChords(hymns_[idx]);
    if (pageIdx > 0) children.push(new Paragraph({ children: [new PageBreak()] }));
    children.push(new Paragraph({
      alignment: AlignmentType.CENTER, spacing: { after: 200 },
      children: [
        new TextRun({ text: `${h.number}. ${h.title}`, bold: true, size: 32, font: "Georgia" }),
        new TextRun({ text: `  (Key: ${h.key || "D"})`, size: 20, color: "8B5500", font: "Courier New" }),
      ],
    }));
    h.lines.forEach((line, li) => {
      if (!line || !line.length) { children.push(new Paragraph({ text: "", spacing: { after: 160 } })); return; }
      let chordText = "", lyricText = "";
      line.forEach((sylObj, si) => {
        const syl = typeof sylObj === "string" ? sylObj : (sylObj?.syl || "");
        const isBreak = typeof sylObj === "object" ? (sylObj?.wordBreak || false) : false;
        const chord = (h.chords[li] && h.chords[li][si]) || "";
        const colWidth = Math.max(syl.length, chord.length, 2) + 1;
        if (isBreak) { chordText += " "; lyricText += " "; }
        chordText += chord.padEnd(colWidth);
        lyricText += syl.padEnd(colWidth);
      });
      children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 0 }, children: [new TextRun({ text: chordText.trimEnd(), font: "Courier New", size: 20, bold: true, color: "8B4500" })] }));
      children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 160 }, children: [new TextRun({ text: lyricText.trimEnd(), font: "Arial Unicode MS", size: 26 })] }));
    });
  });
  const doc = new Document({ sections: [{ children }], styles: { default: { document: { run: { font: "Georgia", size: 24 } } } } });
  const blob = await Packer.toBlob(doc);
  saveAs(blob, "csi-hymns.docx");
}
                ...
                <span key={si} style={{display:"inline-flex",flexDirection:"row"}}>
                  {isBreak && (
                    <span style={{display:"inline-flex",flexDirection:"column",alignItems:"center",width:wordGap}}>
                      <span style={{height:chordH}}/><span style={{fontSize:lyricSize,lineHeight:1.4}}>&nbsp;</span>
                    </span>
                  )}
...
// ─────────────────────────────────────────────────────────────
//  ADMIN — EDITABLE KEY FIELD
// ─────────────────────────────────────────────────────────────
const EditableKeyField = ({ value, onChange }) => {
  const [editing, setEditing] = useState(false);
  const [val, setVal] = useState(value || "D");
  const ref = useRef();
  useEffect(() => { setVal(value || "D"); }, [value]);
  useEffect(() => { if (editing && ref.current) { ref.current.focus(); ref.current.select(); } }, [editing]);
  const commit = () => { const cleaned = val.trim().toUpperCase() || "D"; setVal(cleaned); onChange(cleaned); setEditing(false); };
  if (editing) {
    return (
      <input ref={ref} value={val} onChange={e => setVal(e.target.value.toUpperCase())}
        onKeyDown={e => { if (e.key==="Enter"||e.key==="Escape"||e.key==="Tab") { e.preventDefault(); commit(); } }}
        onBlur={commit} maxLength={4}
        style={{ background:"var(--bg2)", border:"1.5px solid var(--gold)", color:"var(--gold2)", fontFamily:"'JetBrains Mono',monospace", fontSize:".78rem", fontWeight:700, textAlign:"center", borderRadius:6, padding:"3px 8px", outline:"none", width:70, textTransform:"uppercase" }}
      />
    );
  }
  return (
    <span onClick={() => setEditing(true)} title="Click to edit key"
      style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:".78rem", fontWeight:700, padding:"3px 12px", borderRadius:20, background:"#c4922a22", border:"1.5px dashed var(--gold)", color:"var(--gold2)", cursor:"pointer", whiteSpace:"nowrap", transition:"all .14s", userSelect:"none", display:"inline-flex", alignItems:"center", gap:5 }}>
      ✎ Key: {val}
    </span>
  );
};
...
const ChordInputCell = ({ chord, onCommit, onMove }) => {
  ...
  return (
    <div className={`chord-slot${chord ? " has-chord" : ""}${editing ? " focused" : ""}`}
      onClick={() => setEditing(true)} style={{position:"relative",minWidth:22,minHeight:20}}>
      {!editing && chord}
      {editing && (
        <input ref={ref} value={val} onChange={e => setVal(e.target.value.toUpperCase())}
          onKeyDown={e => {
          ...
          style={{ position:"absolute",top:-2,left:"50%",transform:"translateX(-50%)", minWidth:46,background:"var(--bg)",border:"1.5px solid var(--gold)", borderRadius:4,color:"var(--gold2)",fontFamily:"'JetBrains Mono',monospace", fontSize:".68rem",fontWeight:600,textAlign:"center",outline:"none", padding:"2px 4px",zIndex:50,boxShadow:"0 4px 16px #0008" }}
          maxLength={12}
        />
      )}
    </div>
...
const SheetEditor = ({ hymn, onUpdate }) => {
  ...
  const setChord = (li, si, val) => {
    const next = { ...safe, chords: safe.chords.map((r,ri) => ri===li ? r.map((c,ci) => ci===si ? val : c) : r) };
    onUpdate(next);
  };
  const moveFocus = useCallback((li, si, dir) => {
    const lines = safe.lines; let l = li, s = si + dir;
    while (l >= 0 && l < lines.length) {
      const line = lines[l];
      if (!line||!line.length) { l += dir; s = dir>0 ? 0 : (lines[l]?.length||1)-1; continue; }
      if (s >= 0 && s < line.length) return;
      l += dir; if (l < 0 || l >= lines.length) break;
              ...
              return (
                <span key={si} style={{display:"inline-flex",flexDirection:"row"}}>
                  ...
                  <span style={{display:"inline-flex",flexDirection:"column",alignItems:"center",cursor:"pointer"}}>
                    <ChordInputCell chord={chord} onCommit={v => setChord(li,si,v)} onMove={dir => moveFocus(li,si,dir)}/>
...
const ExportModal = ({ open, onClose, hymns, currentIdx, mode }) => {
  ...
  const [chordSize, setChordSize] = useState(11);
  ...
  const doExport = async () => {
    ...
    setExporting(true);
    try {
      if (mode === "pdf") { doPDFExport(hymns, indices, lyricSize, chordSize, lineGap); onClose(); }
      else { await doDOCXExport(hymns, indices); onClose(); }
  ...
  return (
    <Modal open={open} onClose={onClose} title={mode==="pdf"?"🖨 Export PDF":"📄 Export DOCX"} wide
      footer={<><Btn onClick={onClose}>Cancel</Btn><Btn onClick={doExport} variant="primary" disabled={exporting}>{exporting ? "⏳ Preparing…" : mode==="pdf" ? "🖨 Open Print Dialog" : "📄 Download .docx"}</Btn></>}>
      ...
      <div style={{maxHeight:160,overflowY:"auto",background:"var(--bg2)",border:"1px solid var(--border)",borderRadius:"var(--r)",padding:7,marginBottom:18}}>
        {hymns.map((h,i) => (
          <div key={h.id} onClick={() => toggle(i)}
            style={{display:"flex",alignItems:"center",gap:9,padding:"6px 9px",borderRadius:6,cursor:"pointer",marginBottom:3,background:selected.has(i)?"#c4922a18":"transparent",border:`1px solid ${selected.has(i)?"var(--gold)":"transparent"}`}}>
            <input type="checkbox" checked={selected.has(i)} onChange={() => toggle(i)} onClick={e=>e.stopPropagation()} style={{accentColor:"var(--gold)",width:14,height:14}}/>
            <span style={{fontFamily:"'Cinzel',serif",fontSize:".63rem",color:"var(--gold)",minWidth:40}}>#{h.number}</span>
            <span style={{fontSize:".82rem",flex:1}}>{h.title}</span>
            <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:".6rem",color:"var(--muted)"}}>Key: {h.key||"D"}</span>
          </div>
        ))}
      </div>
      {mode === "pdf" && (
        <>
          ...
          {[["Lyric size (pt)", lyricSize, setLyricSize, 10, 40],["Chord size (pt)", chordSize, setChordSize, 8, 30],["Line gap (px)", lineGap, setLineGap, 6, 40]].map(([label,val,set,min,max]) => (
            <div key={label} style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
              <label style={{fontSize:".72rem",color:"var(--muted)",minWidth:130}}>{label}</label>
              <input type="range" min={min} max={max} value={val} onChange={e=>set(+e.target.value)} style={{flex:1,accentColor:"var(--gold)"}}/>
              <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:".72rem",color:"var(--gold2)",minWidth:28,textAlign:"right"}}>{val}</span>
            </div>
          ))}
          <div style={{background:"var(--bg2)",borderRadius:8,padding:10,marginTop:6,fontSize:".68rem",color:"var(--muted)"}}>💡 A new window will open — use your browser's <b style={{color:"var(--cream)"}}>Print</b> dialog to save as PDF.</div>
        </>
      )}
      {mode === "docx" && <div style={{background:"var(--bg2)",borderRadius:8,padding:10,marginTop:6,fontSize:".68rem",color:"var(--muted)"}}>💡 A <b style={{color:"var(--cream)"}}>.docx</b> file will be downloaded.</div>}
      <p style={{fontSize:".7rem",color:"var(--muted)",marginTop:10}}>Selected: <b style={{color:"var(--gold)"}}>{selected.size}</b> hymn(s)</p>
    </Modal>
  );
};
...
const AdminDashboard = ({ hymns, setHymns, onLogout }) => {
  ...
  const [pendingNewHymns, setPendingNewHymns] = useState([]);
  // NEW: key filter for admin sidebar
  const [adminKeyFilter, setAdminKeyFilter] = useState("");
  const [sidebarSearch, setSidebarSearch] = useState("");

  const hymn = currentIdx >= 0 ? ensureChords(hymns[currentIdx]) : null;

  const saveAll = (list) => {
    const sorted = sortHymns(list);
    setHymns(sorted);
    saveStorage("csiHymns", sorted);
    return sorted;
  };

  const save = (updated) => {
    const fixed = ensureChords(updated);
    const next = hymns.map((h,i) => i===currentIdx ? fixed : h);
    const sorted = saveAll(next);
    const newIdx = sorted.findIndex(h => h.id === fixed.id);
    if (newIdx !== currentIdx) setCurrentIdx(newIdx);
  };

  const newHymn = () => {
    const h = ensureChords({ id:uid(), number:hymns.length+1, title:"New Hymn", key:"D", lines:[], chords:[] });
    const sorted = saveAll([...hymns, h]);
    setCurrentIdx(sorted.findIndex(x => x.id === h.id));
  };
  ...
  const doTranspose = (steps) => {
    if (!hymn) return;
    save({ ...hymn, key: transposeChord(hymn.key||"D", steps), chords: hymn.chords.map(row => (row||[]).map(c => c ? transposeChord(c,steps) : "")) });
  };
  ...
  const doImport = () => {
    try {
      ...
      incoming.forEach(h => {
        const existingIdx = hymns.findIndex(e => isDuplicate(e, h));
        if (existingIdx !== -1) duplicates.push({ incoming: h, existingIdx });
        else fresh.push(h);
      });
      if (duplicates.length > 0) {
        saveAll([...hymns, ...fresh]);
        setPendingNewHymns(fresh); setDupQueue(duplicates); setDupQueueIdx(0);
        setImportText(""); setShowImport(false);
      } else {
        const sorted = saveAll([...hymns, ...incoming]);
  ...
  const resolveDup = (action) => {
    ...
    let current = [...hymns];
    if (action === "replace") { current[existingIdx] = ensureChords({ ...incoming, id: current[existingIdx].id }); saveAll(current); }
    else if (action === "keep_both") { current = [...current, ensureChords({ ...incoming, id: uid() })]; saveAll(current); }
    const next = dupQueueIdx + 1;
    if (next >= dupQueue.length) { setDupQueue([]); setDupQueueIdx(0); setPendingNewHymns([]); setCurrentIdx(hymns.length > 0 ? hymns.length - 1 : 0); }
    else { setDupQueueIdx(next); }
  };

  // Filtered sidebar list
  const filteredHymns = hymns.map((h,i) => ({h,i})).filter(({h}) => {
    const matchKey = !adminKeyFilter || (h.key||"D") === adminKeyFilter;
    const matchSearch = !sidebarSearch || h.title.toLowerCase().includes(sidebarSearch.toLowerCase()) || String(h.number).includes(sidebarSearch);
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100vh",background:"var(--bg)"}}>
      <div className="cross-bg"/>
      <header style={{background:"var(--surface)",borderBottom:"1px solid var(--border)",padding:"0 16px",height:52,display:"flex",alignItems:"center",gap:12,flexShrink:0,position:"sticky",top:0,zIndex:100,boxShadow:"0 2px 20px #0008"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:4,flexWrap:"wrap",flex:1}}>
          ...
          <Btn small onClick={() => doTranspose(-1)}>♭↓ Transpose</Btn>
          <Btn small onClick={() => doTranspose(1)}>♯↑ Transpose</Btn>
          <span style={{width:1,height:20,background:"var(--border)",margin:"0 2px"}}/>
          <Btn small onClick={clearChords}>🗑 Chords</Btn>
          <span style={{width:1,height:20,background:"var(--border)",margin:"0 2px"}}/>
          <Btn small variant="primary" onClick={() => { setExportMode("pdf"); setShowExport(true); }}>🖨 Print PDF</Btn>
          <Btn small variant="blue" onClick={() => { setExportMode("docx"); setShowExport(true); }}>📄 Download DOCX</Btn>
        </div>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden",position:"relative",zIndex:1}}>
        {/* ── Sidebar ── */}
        <aside style={{width:268,background:"var(--surface)",borderRight:"1px solid var(--border)",display:"flex",flexDirection:"column",flexShrink:0,overflow:"hidden"}}>
          <div style={{padding:"8px 10px",borderBottom:"1px solid var(--border)",display:"flex",alignItems:"center",justifyContent:"space-between",gap:6}}>
            <span style={{fontFamily:"'Cinzel',serif",fontSize:".6rem",letterSpacing:".1em",color:"var(--muted)",flexShrink:0}}>
              HYMNS <span style={{color:"var(--gold)",marginLeft:4}}>{hymns.length}</span>
            </span>
          ...
          <div style={{padding:"7px 8px",borderBottom:"1px solid var(--border)",display:"flex",flexDirection:"column",gap:5}}>
            <input
              ...
              placeholder="Search…"
              style={{width:"100%",background:"var(--bg2)",border:"1px solid var(--border)",color:"var(--cream)",padding:"5px 8px",borderRadius:6,fontSize:".7rem",fontFamily:"'JetBrains Mono',monospace",outline:"none"}}
            />
            <KeyFilterDropdown hymns={hymns} value={adminKeyFilter} onChange={setAdminKeyFilter} style={{width:"100%"}}/>
          </div>
          <div style={{flex:1,overflowY:"auto",padding:7}}>
            {filteredHymns.map(({h,i}) => (
              <div key={h.id} onClick={() => setCurrentIdx(i)}
                style={{padding:"7px 10px",borderRadius:"var(--r)",cursor:"pointer",marginBottom:3,border:`1px solid ${i===currentIdx?"var(--gold)":"transparent"}`,background:i===currentIdx?"#c4922a18":"transparent",transition:"all .14s"}}>
                <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:2}}>
                  <span style={{fontFamily:"'Cinzel',serif",fontSize:".58rem",color:"var(--gold)"}}>#{h.number}</span>
                  <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:".55rem",padding:"1px 6px",borderRadius:8,background:"var(--surface2)",border:"1px solid var(--border2)",color:"var(--muted)"}}>{h.key||"D"}</span>
                </div>
                <div style={{fontSize:".8rem",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{h.title}</div>
              </div>
            ))}
            {filteredHymns.length === 0 && <p style={{textAlign:"center",color:"var(--muted)",fontSize:".72rem",padding:"20px 0"}}>No hymns match filter</p>}
          </div>
          {/* Quick import */}
          <div style={{padding:10,borderTop:"1px solid var(--border)"}}>
            ...
            <textarea value={importText} onChange={e=>setImportText(e.target.value)}
              ...
              style={{width:"100%",height:68,background:"var(--bg2)",border:"1px solid var(--border)",color:"var(--cream)",padding:7,borderRadius:"var(--r)",fontSize:".68rem",fontFamily:"'JetBrains Mono',monospace",resize:"vertical",outline:"none",marginBottom:6}}/>
        ...
        <main style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"10px 20px",borderBottom:"1px solid var(--border)",display:"flex",alignItems:"center",gap:10,background:"var(--surface)",flexShrink:0}}>
            <div style={{display:"flex",alignItems:"center",gap:4,flexShrink:0}}>
              <span style={{fontFamily:"'Cinzel',serif",fontSize:".62rem",color:"var(--muted)"}}>#</span>
              <input type="number" value={hymn?.number || ""}
                onChange={e => { const num = parseInt(e.target.value)||hymn.number; save({...hymn, number: num}); }}
                style={{width:52,textAlign:"center",fontFamily:"'Cinzel',serif",fontSize:".85rem",background:"var(--bg2)",border:"1px solid var(--border2)",color:"var(--gold)",outline:"none",borderRadius:6,padding:"3px 4px"}}
              />
            </div>
            <div style={{flex:1,position:"relative"}}>
              <input value={hymn?.title || ""} onChange={e => save({...hymn, title: e.target.value})} placeholder="Hymn Title…"
                style={{width:"100%",background:"var(--bg2)",border:"1.5px solid var(--border2)",color:"var(--gold2)",fontFamily:"'Cinzel',serif",fontSize:".92rem",outline:"none",borderRadius:8,padding:"6px 12px",transition:"border-color .15s"}}
                onFocus={e => e.target.style.borderColor="var(--gold)"} onBlur={e => e.target.style.borderColor="var(--border2)"}
              />
              <span style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",fontSize:".6rem",color:"var(--muted)",pointerEvents:"none",fontFamily:"'JetBrains Mono',monospace"}}>✎</span>
            </div>
            {hymn && <EditableKeyField value={hymn.key || "D"} onChange={newKey => save({ ...hymn, key: newKey })}/>}
          </div>
          ...
          <div style={{padding:"6px 20px",background:"var(--surface)",borderBottom:"1px solid var(--border)",display:"flex",alignItems:"center",gap:6,flexWrap:"wrap",flexShrink:0}}>
            <span style={{fontFamily:"'Cinzel',serif",fontSize:".6rem",letterSpacing:".1em",color:"var(--muted)"}}>PALETTE →</span>
            {chips.map(ch => (
              <span key={ch} style={{fontFamily:"'JetBrains Mono',monospace",fontSize:".68rem",fontWeight:600,padding:"2px 8px",borderRadius:4,background:"var(--surface2)",border:"1px solid var(--border2)",color:"var(--gold)",cursor:"default",userSelect:"none"}}>{ch}</span>
            ))}
          ...
          <div style={{flex:1,overflowY:"auto",padding:"28px 40px"}}>
            {!hymn ? (
              ...
              <div style={{maxWidth:820,margin:"0 auto"}}>
                <h2 style={{fontFamily:"'Cinzel',serif",fontSize:"1.3rem",color:"var(--gold2)",textAlign:"center",marginBottom:6,letterSpacing:".08em"}}>{hymn.title}</h2>
                <p style={{textAlign:"center",marginBottom:24,fontFamily:"'JetBrains Mono',monospace",fontSize:".65rem",color:"var(--muted)"}}>Hymn #{hymn.number} · Key: {hymn.key||"D"}</p>
                <SheetEditor hymn={hymn} onUpdate={save}/>
              </div>
            )}
          </div>

          <div style={{padding:"4px 20px",background:"var(--surface)",borderTop:"1px solid var(--border)",display:"flex",gap:14,flexWrap:"wrap",fontFamily:"'JetBrains Mono',monospace",fontSize:".6rem",color:"var(--muted)",flexShrink:0}}>
            {[["Click syllable","open chord input"],["Enter / →","next"],["←","prev"],["Esc","close"],["✎ Key","click to edit"],["Title","always editable"]].map(([k,v])=>(
      ...
      <Modal open={showImport} onClose={() => setShowImport(false)} title="📥 Import Hymn JSON"
        ...
        <p style={{fontSize:".72rem",color:"var(--muted)",marginBottom:10}}>
          Paste a single hymn or an array. <b style={{color:"var(--gold2)"}}>Same number + same title + same key</b> = duplicate. <b style={{color:"var(--gold2)"}}>Different key</b> = accepted as new entry.
        </p>
        <textarea value={importText} onChange={e=>setImportText(e.target.value)}
      ...
      {dupQueue.length > 0 && dupQueueIdx < dupQueue.length && (() => {
        ...
        return (
          <Modal open={true} onClose={() => {}} title="⚠ Duplicate — Same Number, Title & Key"
            footer={
              <div style={{display:"flex",gap:8,justifyContent:"flex-end",flexWrap:"wrap"}}>
                <Btn variant="danger" onClick={() => resolveDup("delete")}>🗑 Skip</Btn>
                <Btn onClick={() => resolveDup("keep_both")}>📋 Keep Both</Btn>
                <Btn variant="primary" onClick={() => resolveDup("replace")}>🔄 Replace</Btn>
              </div>
            }>
            <p style={{fontSize:".75rem",color:"var(--muted)",marginBottom:14}}>{dupQueueIdx+1} of {dupQueue.length}</p>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              <div style={{background:"var(--bg2)",border:"1px solid var(--border)",borderRadius:"var(--r)",padding:12}}>
                ...
                <div style={{color:"var(--muted)",fontSize:".7rem",marginTop:4}}>Key: {existing.key}</div>
              </div>
              <div style={{background:"var(--bg2)",border:"1px solid var(--gold)",borderRadius:"var(--r)",padding:12}}>
                ...
                <div style={{color:"var(--muted)",fontSize:".7rem",marginTop:4}}>Key: {incoming.key}</div>
              </div>
            </div>
          </Modal>
        );
...
// ─────────────────────────────────────────────────────────────
//  USER — FLOW SLIDE VIEWER (with left sidebar + side arrows)
// ─────────────────────────────────────────────────────────────
const FlowSlideViewer = ({ flow, hymns, onBack, onEditName }) => {
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--bg)",display:"flex",flexDirection:"column",position:"relative"}}>
      <div className="cross-bg"/>
      {/* Header */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 16px",background:"var(--surface)",borderBottom:"1px solid var(--border)",zIndex:10,flexShrink:0}}>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        <div style={{flex:1,display:"flex",alignItems:"center",overflow:"hidden",position:"relative"}}>
          ...
          {/* Hymn content */}
          <div key={animKey}
            style={{flex:1,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"32px 80px",overflowY:"auto",height:"100%",animation:`${dir>0?"slideInR":"slideInL"} .3s ease`}}>
            <div style={{maxWidth:820,width:"100%"}}>
              ...
              <h2 style={{fontFamily:"'Cinzel',serif",fontSize:"1.5rem",color:"var(--gold2)",textAlign:"center",marginBottom:28,letterSpacing:".08em",lineHeight:1.3}}>{hymn.title}</h2>
              <HymnViewer hymn={hymn} lyricSize={24} chordSize={16} lineGap={22}/>
            </div>
          </div>
...
const FlowEditor = ({ flow, hymns, onSave, onClose }) => {
  ...
  // Filter unique numbers for picker
  ...
  const handlePickerSelect = (num) => {
    const group = hymnGroups[num];
    if (!group) return;
    // If key filter active, pick the matching key automatically
    if (pickerKeyFilter) {
...
// ─────────────────────────────────────────────────────────────
//  USER — HYMN SLIDE VIEWER (single hymn browse)
// ─────────────────────────────────────────────────────────────
const HymnSlideViewer = ({ hymns, initialHymnId, onBack }) => {
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--bg)",display:"flex",flexDirection:"column",position:"relative"}}>
      <div className="cross-bg"/>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 16px",background:"var(--surface)",borderBottom:"1px solid var(--border)",flexShrink:0,zIndex:10}}>
        <BigBtn onClick={onBack} variant="ghost" icon="←">Back</BigBtn>
        <span style={{fontFamily:"'Cinzel',serif",fontSize:".78rem",color:"var(--gold2)",letterSpacing:".06em",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:300}}>{hymn.number}. {hymn.title}</span>
        <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:".68rem",padding:"3px 10px",borderRadius:20,background:"#c4922a22",border:"1px solid var(--gold)",color:"var(--gold2)"}}>Key: {hymn.key||"D"}</span>
      </div>
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",position:"relative",overflow:"hidden"}}>
        ...
        <div key={animKey} style={{flex:1,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"36px 80px",overflowY:"auto",height:"100%",animation:`${dir>0?"slideInR":"slideInL"} .3s ease`}}>
...
const UserDashboard = ({ hymns, onLogout }) => {
  ...
  const handleHymnClick = (group) => {
    // If key filter active and one hymn matches, go directly
    if (keyFilter) {
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--bg)",display:"flex",flexDirection:"column"}}>
      ...
      <header style={{background:"var(--surface)",borderBottom:"1px solid var(--border)",padding:"0 20px",height:58,display:"flex",alignItems:"center",gap:12,flexShrink:0,boxShadow:"0 2px 20px #0008",zIndex:10,position:"sticky",top:0}}>
        ...
        <BigBtn variant="ghost" onClick={onLogout} icon="←">Exit</BigBtn>
      </header>
          ...
          <div style={{maxWidth:720,margin:"0 auto"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {groupedList.map(group => {
                const rep=group[0];
                // Show all keys, highlight filtered key
...
export default function App() {
  ...
  const [hymns, setHymns] = useState(() => {
    ...
    if (stored && stored.length > 0) return sortHymns(stored.map(ensureChords));
    const d = ensureChords({ ...DEMO_HYMN });
